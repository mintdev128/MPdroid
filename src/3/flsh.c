...
//# ram + swap
    ...
    unsigned long long mem_mb = total_bytes / (1048576);
    #ifdef __FreeBSD__
    long swap_total;
...
void Flatsh_menu_mode(void){
    ...
    //# clear screen and start menu mode (ncurses) 
        ...
        mvprintw(1,cols-5-strlen(loc),"%s",loc);
...
void Flatsh_config_mode(void){
    ...
    char *guide[] = {
        ...
        "   echo $PATH             ",
        "If you get output (it should look like /a:/b/c:/d/e/f) you can select one of them",
        ...
        "After this, if you want to make it your default shell run",
        "   echo /bin/flsh >> /etc/shells",
        "To add this in to shells list, then",
        "   chsh -s /bin/flsh",
        "As my teacher said \"remember for the rest of your life\" to run it twice,",
        "first to create config, and second to edit config files, to do it faster you can just run",
        "   flsh && flsh",
        ...
        switch(c) {
            ...
            case 10:
                ...
                if (choice == 0) {
                    ...
                    mvprintw(lines-3,2,"FlatOS(0),BSDCFG(1),Black&Gray(2)");
                    ...
                    mvprintw(lines-3,2,"CP437(0),ASCII(1),SLASH(2)");
...
int main() {
    ...
    if (cthm == NULL) {
        printf("flatsh:couldn\'t open themes file");
        FILE *cthm = fopen(thmp, "w");
        printf("Boot this program second time to finish config");
        fclose(cthm);};
    if (cbor == NULL) {
        printf("flatsh:couldn\'t open borders file");
        FILE *cbor = fopen(borp, "w");
        printf("Boot this program second time to finish config");
        fclose(cbor);};
    if (fgets(line, sizeof(line), cthm)==NULL){
        printf("flatsh:couldn\'t find theme file contents\nPress enter to continue to config mode");
        getchar();
        Flatsh_config_mode();};
